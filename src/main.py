...
imgRGB = cv.imread(cv.samples.findFile("Dataset/nickr.jpg"))
...
for contour in contours:
    ...
    # new value
    if contourn_area >= imgRGBs.size*0.001:
        x = approx.ravel()[-2]
        y = approx.ravel()[-1]
        if len(approx) == 8:
            cv.drawContours(imgRGBs,contour,-1,(0,255,0),2,cv.LINE_AA)
            cv.putText(imgRGBs, "Placa", (x,y), cv.FONT_HERSHEY_COMPLEX, 0.5, (0,0,0))

# Show images
# cv.imshow("Display window HSV", imgHSV)
# cv.imshow("Display window RGB", imgRGBs)
cv.imshow('full-mask',full_mask)
cv.imshow('Imagem final',imgRGBs)
